...
int main(int argc, char ** argv)
{
    ...
    for (int i = 0; i < k; ++i)
    {
        ...
        connection->send(thread_board, m, left_thread, 2 * i);
        connection->send(thread_board + (size - 1) * m, m, right_thread, 2 * i + 1);
        
        connection->recv(top, m, left_thread, 2 * i + 1);
        connection->recv(bottom, m, right_thread, 2 * i);
